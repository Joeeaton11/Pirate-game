...
## Yes — split into batches, request separately

🔒 **One asset per image — never a multi-item grid or catalog sheet.** Every numbered item below,
in every batch, is its own separate image generation with its own prompt. "Batch" is a checklist
grouping only, to help you order the work — not a delivery format. Never ask the generator for
"all of Batch 1 in one image."

Six batches below, ordered by how much of the app each one touches. Batches 1–3 are the ones worth
...
appears). Batches 4–6 are lower-frequency, higher-detail pieces — fine to commission later, or skip
initially and ship with emoji a while longer if you want to pace the work.

## Paste this once, reuse for every item
...
> flat-icon style.
>
> **This is a single UI asset, not a scene and not a sheet** — one object (or, only where the item
> itself is inherently a small set, like a 3-frame button state group explicitly called out below,
> those three states together) on a plain background, not an environment, and never multiple
> unrelated items packed into one canvas for later cutting. Consistent light source (upper-left).
>
...
>
> Now generate this one specific item: [paste one entry below]

---

## Batch 1 — Buttons & Panels (highest priority — every screen uses these)
...
## Batch 3 — Icons: Currency, Resources, Stats & Specialties
...
## Batch 4 — Badges, Ranks & Quest Markers
...
## Batch 5 — Header & Navigation Chrome
...
natural fit for Batch 5's back/menu button or a map-screen compass widget). Worth checking these
against whatever new pieces come in for style consistency before finalizing a look.

## Suggested order

Batch 1 (buttons/panels) and Batch 3 (icons) first — cheapest, and every other screen in the app
immediately looks less placeholder-y once those two land. Batch 2 (bars) next, since HP/heat are
on-screen constantly during play. Batches 4-6 whenever — they're real improvements but lower
