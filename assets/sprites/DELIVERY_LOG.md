...
| 2026-08-21 | `assets/brand/tileset-catalog/terrain_extras_3_sheet_v1.png` | `TERRAIN_EXTRAS_3_MANIFEST.md` | 311 | `tiles/` (ground, paths, paving, transitions, beach, elevation, bridges, water), `nature/` (trees, rocks, vegetation), `props/`, `decals/`, `water_fx/`, `harbour/`, `buildings/` (new `floor_tile_*`), `weather_fx/` (first entries — previously empty) | **No** — cut and filed only, not yet wired into any renderer. Re-cut twice on 2026-08-21: round 1 fixed baked-in caption text, wrong category boundaries, and a miscounted Panel 13/22 structure found by an independent `asset-qa` pass (306→309 items); round 2, a second independent QA pass on round 1's own fixes, found Panels 4/10/11 mislabeled (correct crops, wrong category slots) and Panel 13's Palms/Broadleaf undercounted/fragmented (309→311 items). See `TERRAIN_EXTRAS_3_MANIFEST.md`'s intro and `GAME_DESIGN.md` items 145–146 |
