...
Source: `assets/brand/tileset-catalog/terrain_extras_sheet_v1.png` — the original, uncut
1536×1024 delivery ("Additional Ground/Water/Road/Edges/Cliffs/Vegetation/Stairs/Walls (Extra)" +
two hero feature sets + hero landmarks + misc overlays), delivered 2026-08-17, kept in the repo
alongside `master_catalog_v1.png`/`tortuga_focus_v1.png` (same folder, same convention — original
reference sheets a cut pass came from, not meant to be used directly in-game). Files below use the
codebase's plain-sequential-number convention (`{descriptor}_{n}.png`) rather than baking each
specific variant name into the filename — this doc is the lookup table from filename back to what
it actually shows.

**Why the source sheet is kept, not just the cut sprites:** (1) re-verification — if a cut sprite
ever looks wrong (clipped, wrong content, style mismatch), the original panel it came from is
right here to check against, rather than trusting the crop blind; (2) future requests — if more
items in this same style/theme are ever needed (e.g. a few more ground variants, another cliff
formation), the sheet shows the exact visual language to match, along with every panel's own
labels, at full source resolution. Each section below now names the source panel's own roman
numeral (e.g. "Panel V") so you can go straight to the right part of the sheet.
...
## `water_fx/water_fx_extra_1..9` — Water Variants (dynamic effects) (source: Panel II)
...
## `props/fence_wall_extra_1..16` — Walls, Fences & Barriers (source: Panel VIII)
...
## `decals/ground_decal_1..16` — Misc Details & Overlays (source: Panel XII)
...
## Hero pieces (source: Panels IX–XI)

### Panel IX — Majestic Waterfall (hero feature set)

**`landmarks/waterfall_complete_1.png`** — sub-section A, the assembled multi-tier waterfall hero
illustration.

**`water_fx/waterfall_piece_1..14`** — sub-section B. 1 Top Source · 2 Top Source (alt) ·
3 Waterfall Wide · 4 Waterfall Medium · 5 Waterfall Narrow · 6 Cascade Short · 7 Cascade Tiny ·
8 Splash Large · 9 Splash Medium · 10 Splash Small · 11 Spray Mist · 12 Impact Pool ·
13 Foam Eddy · 14 Foam Edge

**`water_fx/water_spout_1..7`** — sub-section C. 1 Spout Tall · 2 Spout Medium · 3 Spout Short ·
4 Geyser Burst · 5 Water Ripple Ring · 6 Mist Puff · 7 Dripping Rocks

### Panel X — Neptune Fountain (hero landmark set)

**`landmarks/fountain_complete_1.png`** — sub-section A, the assembled Neptune Fountain hero
illustration.

**`props/fountain_piece_1..6`** — sub-section B. 1 Base Corner · 2 Base Straight · 3 Pool Empty ·
4 Pool Water · 5 Pool Ripple · 6 Base Corner (Alt Angle). Sub-section B's own labels on the sheet
only name 5 items, but the real pixel content segments cleanly into 6 distinct pieces — item 6 is
a second curved base-wall piece at a different angle, visually similar to item 1 but genuinely
separate art. Kept as its own sprite rather than merged into item 1 to force a match to the
5-label count.

**`landmarks/fountain_statue_1..5`** — sub-section C. 1 Neptune Statue · 2 Side Trident ·
3 Side Conch · 4 Side Dolphin · 5 Sea Creature Decor

**`water_fx/fountain_jet_1..3`** — sub-section D. 1 Water Jet Tall · 2 Water Jet Medium ·
3 Water Jet Short

### Panel XI — Hero Environmental Landmarks (large props / island features)
