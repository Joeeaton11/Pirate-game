...
| `characters/` | NPCs beyond Scally: crew, named NPCs, generic townsfolk/pirates/sailors/etc. Use filename prefixes (`crew_01_idle.png`, `generic_sailor_drunk_1.png`) rather than a folder per archetype | **Started** (2026-08-29) — `admiral_grace_portrait_1.png` (user-generated, real per-item alpha cut in-session — see `characterSprites.ts`'s doc comment) wired into `GraceScreen.tsx`'s header as a head-and-shoulders bust crop, replacing `GRACE_EMOJI`. Mirrored to face screen-left, ahead of the still-pending swap to `ConversationBox`'s two-sided portrait layout (Scally left/facing right, NPCs right/facing left) — not yet done, GraceScreen still shows all dialogue as stacked cards rather than tap-through. Everyone else (Blackfin, the 6 Pirate Lords, 12 crew, 6 threats) is still emoji |
